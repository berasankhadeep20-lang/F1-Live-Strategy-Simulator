    ...
    <div className="app">
      ...
      <header className="app-header">
        <div className="header-left">
          ...
          <span className="header-desc">Live pit strategy · tyre deg · undercut analysis</span>
        </div>
        <div className="header-right">
          <a href="https://github.com/berasankhadeep20-lang/F1-Live-Strategy-Simulator" target="_blank" rel="noopener noreferrer" className="github-link">
      ...
      {/* Safety car banner */}
      {(sim.activeSafetyCar || selectedRace.safetyCarEvents.some(e => e.startLap > sim.currentLap && e.startLap <= sim.currentLap + 3)) && (
        <SafetyCarBanner activeSafetyCar={sim.activeSafetyCar} currentLap={sim.currentLap} race={selectedRace} />
      )}
      ...
      <div className="playback-bar">
        <PlaybackControls
          ...
          onShare={() => { sim.generateShareURL(); setShowShare(true); }}
          onKeyboardHelp={() => setShowKeyHelp(true)}
        />
      </div>
      ...
      <div className="main-layout">
        ...
        <aside className="right-panels">
          ...
          />
          <WeatherPanel
            rainProbability={sim.rainProbability}
            onRainChange={sim.setRainProbability}
            race={selectedRace}
            currentLap={sim.currentLap}
          />
        </aside>
      ...
      {/* Modals */}
      {showShare && (
        <ShareModal url={sim.shareURL} onClose={() => setShowShare(false)} onGenerate={sim.generateShareURL} />
      )}
      {showKeyHelp && <KeyboardShortcutsHelp onClose={() => setShowKeyHelp(false)} />}
    </div>
  );
}
